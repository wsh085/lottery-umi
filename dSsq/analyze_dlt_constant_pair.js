/**
 * 大乐透恒值数配对法验证脚本
 * 规则：每期前区红球中，存在两个号码相加等于36
 * 大乐透前区：35选5，理论配对共17对：
 * (1,35),(2,34),(3,33),(4,32),(5,31),(6,30),(7,29),(8,28),(9,27),
 * (10,26),(11,25),(12,24),(13,23),(14,22),(15,21),(16,20),(17,19)
 * 注：18+18=36，但同一号码不重复，不计入
 */
...
/**
 * 解析红球字符串为数字数组
 * @param {string} redStr - 如 "05 08 16 17 27"
 * @returns {number[]}
 */
...
function analyzeFile(filePath, label) {
  ...
  for (const item of data) {
    ...
    if (pairs.length > 0) {
      ...
      hitDetails.push({
        ...
        配对: pairs.map(
          ([a, b]) =>
            `${a.toString().padStart(2, "0")}+${b.toString().padStart(2, "0")}=36`,
        ),
        配对数: pairs.length,
      });
    }
  ...
  return {
    ...
    totalPairs,
    avgPairsPerHit: hitCount > 0 ? (totalPairs / hitCount).toFixed(2) : "0.00",
    pairFrequency,
    hitDetails,
  };
}
...
function printResult(result) {
  console.log("\n" + "=".repeat(70));
  console.log(`  ${result.label} 大乐透恒值数配对法验证结果（和值=36）`);
  ...
  sorted.forEach(([pair, count], idx) => {
    ...
    console.log(
      `  ${(idx + 1).toString().padStart(2, "0")}. ${pair} = 36  出现 ${count.toString().padStart(3, "0")} 次  ${bar}`,
    );
  });
  ...
  result.hitDetails.slice(-20).forEach((d) => {
    console.log(
      `  期数 ${d.期数} | 红球: ${d.红球} | 蓝球: ${d.蓝球} | ${d.配对.join(", ")}`,
    );
  });
  console.log("=".repeat(70));
}
...
const file2025 = path.join(dataDir, "da_2025_data.json");
const file2026 = path.join(dataDir, "da_2026_data.json");
...
const pairsAll = result2025.totalPairs + result2026.totalPairs;
console.log("\n" + "=".repeat(70));
...
console.log(`  总配对出现次数：${pairsAll} 次`);
console.log(`  命中期平均配对数：${(pairsAll / hitAll).toFixed(2)} 对/期`);
console.log("=".repeat(70) + "\n");
