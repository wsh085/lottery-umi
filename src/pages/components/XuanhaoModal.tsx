...
interface XuanhaoModalProps extends Omit<ModalProps, "onOk" | "onCancel"> {
  /** 获取当前杀号 */
 ...
 * @returns
 */
const XuanhaoModal = (props: XuanhaoModalProps) => {
  const [xuanhao, setXuanhao] = useState<number[]>([]);
  const currentShaHao = useMemo(
    () => props.handleShaHao({ showModal: false }),
    [props.open]
  );
