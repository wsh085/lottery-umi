...
function prepareData(historyData, sequenceLength = 5) {
  ...
  for (let i = sequenceLength; i < historyData.length; i++) {
    ...
    try {
      ...
      const sequenceFeatures = sequence.map((item) => {
        ...
        // 归一化
        const normalizedRed = validRedNumbers.map(
          (num) => (num - RED_MIN) / (RED_MAX - RED_MIN)
        );
      ...
      // 归一化标签
      const normalizedNextRed = validNextRed.map(
        (num) => (num - RED_MIN) / (RED_MAX - RED_MIN)
      );
  ...
  const hasInvalidData =
    ...
    labels.some((label) =>
      label.some((val) => isNaN(val) || val === undefined)
    );
...
async function predictNextNumbers(historyData) {
  try {
    ...
    // 计算加权平均预测值
    const weights = Array(PREDICTION_TIMES)
      .fill(1)
      .map((_, i) => 1 + i / PREDICTION_TIMES);
    ...
    // 处理预测结果
    const predictedRed = avgPrediction
      .slice(0, 5)
      .map((num) => Math.round(num * (35 - 1) + 1))
      .filter((num) => num >= 1 && num <= 35);

    const predictedBlue = avgPrediction
      .slice(5, 7)
    ...
    if (confidence < MIN_CONFIDENCE) {
      console.log(
        `置信度 ${confidence.toFixed(2)}% 不足 ${MIN_CONFIDENCE}%，重新预测...`
      );
    ...
    (predictions.reduce((acc, pred) => {
      const redMatch =
        pred
          .slice(0, 5)
          .map((num) => Math.round(num * (35 - 1) + 1))
          .filter((num) => result.predictedRed.includes(num)).length / 5;

      const blueMatch =
        pred
          .slice(5, 7)
          .map((num) => Math.round(num * (12 - 1) + 1))
          .filter((num) => result.predictedBlue.includes(num)).length / 2;
